...
if st.sidebar.button("Add Website"):
    if custom_url:
        ...
        if success:
            st.sidebar.success("Website added! Starting immediate scraping...")
            
            # Immediately scrape the new website
            with st.sidebar.container():
                with st.spinner("🕷️ Scraping new website..."):
                    try:
                        new_scholarships = scraper.scrape_single_site(
                            custom_url, 
                        ...
                        if new_scholarships:
                            st.sidebar.success(f"✅ Found {len(new_scholarships)} scholarships!")
                            # Add to session state for immediate display
                            if 'scraped_data' not in st.session_state:
                                st.session_state.scraped_data = []
                            st.session_state.scraped_data.extend(new_scholarships)
                        else:
                            st.sidebar.info("🔍 No scholarships found on this site yet")
...
with st.container():
    ...
    with col2:
        search_clicked = st.button("⚡ Quick Search", use_container_width=True, help="Search cached results (instant)")
    with col3:
        aggressive_search = st.button("🚀 Live Scrape", use_container_width=True, help="Real-time scraping (slower but fresh)")

if search_clicked or aggressive_search:
